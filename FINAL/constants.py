...
# Graph data contains the distance between the nodes/
# events (measured in milliseconds), as well as the direction to take
# to reach the node.
# Also contains special cases where the robot's orientation changes
# without the robot taking an explicit turn (left/right).
...
# Special forward instruction is used when the bot needs to stop at an
# event after being pinged by the computer. The computer uses distance from
# AruCo marker to determine if the robot is at an event.
SPL_FWD_INSTRUCTION = "x"
SPL_SLOW_FWD = "X"

# This dictionary is used to assign priority to the labels
# of the events. Lower number is higher priority.
label2priority = {
    "fire": 1,
...
* Input:            pose: int (either 0, 1, 2, or 3)
* Output:           int (0, 1, 2, 3) represent reverse of the input `pose`.
