...
for epoch in tqdm(range(EPOCHS)):
    ...
    epoch_cpu = timer()
    tt = print_train_time(start=epoch_cpu_st, end=epoch_cpu, device=str(next(model.parameters()).device))
...
model_1_results = eval_model(model=model, data_loader=test_dataloader, loss_fn=loss_fn, accuracy_fn=accuracy_fn)
print(model_1_results)

# save model
torch.save(model.state_dict(), "models/pyTorch-CNN-pjr-2710-01.pt")
