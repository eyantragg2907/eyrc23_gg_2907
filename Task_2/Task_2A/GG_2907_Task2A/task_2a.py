...
# Team ID:			2907
# Author List:		Abhinav Lodha, Subham Jalan
# Filename:			task_2a.py
...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    for i in range(len(markerIds)):
        ArUco_corners[int(markerIds[i][0])] = markerCorners[i][0]
    
    ArUco_details_dict = ArUco_corners.copy()

    for i in ArUco_corners:
        x1, y1 = ArUco_corners[i][3]
        x2, y2 = ArUco_corners[i][0]
        
        angle = math.degrees(math.acos((y1 - y2) / np.linalg.norm(ArUco_corners[i][3] - ArUco_corners[i][0])))
        if x2 - x1 >= 0:
            angle *= -1
        center = list((ArUco_corners[i][3]+ ArUco_corners[i][1])/2)
        center = [int(i) for i in center]
        ArUco_details_dict[i] = [center, int(angle)]
